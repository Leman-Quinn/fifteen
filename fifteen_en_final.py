            ...
            for rows in range(int(num_rows)): #generates the board
                ...
                for columns in range(int(num_columns)):
                    if current_number<total_pieces:
                        ...
                        rows.append(0)
                board.append(rows)
                win_board.append(list(rows)) #Crea una nueva lista en win_grid
            randomizer(board, num_rows, num_columns)
            printer(board, num_rows, num_columns)
            while True:
                if board==win_board and moves_counter<=(Q*5): #checks for winning conditions
                    win(moves_counter)
                    ...
                    if user_movement=="1":
                        ...
                        close()
                elif board!=win_board and moves_counter<(Q*5):
